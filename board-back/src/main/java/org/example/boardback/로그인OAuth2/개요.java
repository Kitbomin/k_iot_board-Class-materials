 ...
 * > 비밀번호 제공 없이, (네이버, 구글, 카카오 등)일부 기능만 사용할 수 있는 임시 토큰을 발급해주는 방식
 *
 * === OAuth2 사용 흐름 ===
 * 1) 프론트에서 "소셜 계정 로그인" 버튼 클릭 시
 *  : 브라우저가 백엔드의 /oauth2/authorization/{sns-name} 으로 이동함
 *
 * 2) Spring Security가 자동으로 소셜 로그인 페이지로 리다이렉트 시킴
 *
 * 3) 사용자가 소셜 로그인을 완료하면
 *  : 소셜 계정이 우리 서버의 /login/oauth2/code/{sns-name} 으로 코드를 보냄
 *
 * 4) 서버 쪽 CustomOAuth2UserService가 코드를 AccessToken 으로 교환하고, 사용자 정보 (이메일/닉네임) 를 가져옴
 *
 * 5) 우리 DB에 해당 이메일의 유저가 있으면 -> 로그인이 됨
 *                         유저가 없으면 -> 자동 회원가입 후 로그인을 시킴
 *
 * 6) 로그인에 성공하면 OAuth2AuthenticationSuccessHandler 에서
 *      - 프로젝트 서비스용 JWT Access/Refresh Token 생성
 *      - 프론트 콜백 URL로 리다이렉트 시킬거임
