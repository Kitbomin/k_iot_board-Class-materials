...
public class AuthServiceImpl implements AuthService {
    ...
    public ResponseDto<LoginResponseDto> login(LoginRequestDto request) {
        try {
            ...
            // : ID가 존재하는지 (username이 존재하는지)
            // : 해당 ID의 비밀번호가 맞는지
            // >> 전부 내부에서 검사함 (틀리면 BadCredentialsException이 발생함)
            ...
            // : 즉 이 계정은 현재 해당 RefreshToken을 가진 상태다 < 라는 것을 DB에 기록함
    ...
    public ResponseDto<LoginResponseDto> refresh(RefreshRequestDto request) {
        ...
        // 전달받은 refreshToken 문자열을 꺼냄 => DB에 있는거랑 일치하는지 확인하기 위해
        ...
        // 새로운 Access/Refresh Token 생성 + RefreshToken 엔티티 갱신
    ...
    public ResponseDto<SignupResponseDto> signup(SignupRequestDto request) {
        ...
        if (userRepository.findByUsername(request.username()).isPresent()) {
            // 기존에 해당 아이디가 있으면 예외 발생 => 기존값이랑 일치하면 안됨 => 회원가입이니까.
