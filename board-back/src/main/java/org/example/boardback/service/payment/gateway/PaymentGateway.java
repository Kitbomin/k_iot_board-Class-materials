...
/** === PG 연동을 추상화한 인터페이스 ===
 * 결제 게이트웨이 공통 인터페이스
 * - 실제 PG(토스, 카카오 등) 또는 모의 결제(MOCK) 같은 형태를 추상화하는 인터페이스
 * - 새로운 결제 수단이 추가되더라도 해당 결제수단 PaymentService가 해당 인터페이스에 의존하게 됨
 */
public interface PaymentGateway {
     ...
     *
     * +) "준비 + 승인" 과정을 해당 메서드 내부에서 한번에 처리함
     *      >> 실제 프로덕션에서는 주로 결제 준비, 결제 승인 2 단계로 나눔
     */
