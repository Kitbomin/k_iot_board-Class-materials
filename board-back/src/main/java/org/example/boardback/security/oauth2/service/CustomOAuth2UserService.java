...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    // User Entity 를 Spring Security 에서 사용하는 UserPrincipal로 변환하는 매퍼
    private final UserPrincipalMapper userPrincipalMapper;

    /**
     * OAuth2 로그인 시 provider(google, kakao, naver) 로부터 사용자 정보를 가져와 프로젝트 서비스의 User 엔티티로 저장 / 업데이트 한 뒤에
     * >> 최종적으로 Spring Security가 이해할 수 있는 OAuth2User(UserPrincipal) 형태로 리턴
     */
    @Override
    @Transactional
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        // 1. provider 로부터 사용자 정보를 가져오기
        ...
        // 5. 프로젝트 서비스에서 사용하는 UserPrincipal 래핑
    ...
    protected User upsertUser(AuthProvider provider, OAuth2UserInfo userInfo) {
        // 소셜 서비스에서 제공하는 고유 사용자 ID (구글 = sub, 카카오 = id ...)
        ...
        // - 다른 provider 안에는 providerId가 같더라도 구분 가능함
        ...
        return userRepository.findByProviderAndProviderId(provider, providerId)
                ...
                .orElseGet(() -> {
                    // 2) 처음 로그인 하는 유저인 경우 -> 새로운 User Entity 생성을 해야함
    ...
    // cf) upsert = UPDATE + INSERT 합쳐놓은 mySQL
}
