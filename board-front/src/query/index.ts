...
//            ex) 로그인 여부, 모달 열림/닫힘 상태, 현재 선택된 앱 등을 관리함
//                폼 임시값, ui 모드(다크/라이트), 장바구니 상태 등등등
...
//      4) 캐싱하고 싶으면 직접 전역에 저장시켜야함
//      5) 페이지 이동 후 다시 돌아올 경우 fetch부터 재시작 => 1번부터 다시해야함
//      ===>> 이걸 다 해주는게 React Query임 (신세계잖아?!)

//? react-query의 "서버 상태"
//  1) 진짜 소스는 서버(DB)에 있고, 프론트는 "복사본"을 가짐
//  2) 여러 사용자, 여러 클라이언트에서 동시 수정이 가능함
//  3) 프론트에서 실제 데이터를 100% 통제할 수 없음
//  4) 항상 서버에서 다시 fetch해서 맞춰줘야함 (일종의 동기화)
...
const {data, isLoading, isError, isSuccess, status, error} = useQuery({
  ...
  staleTime: 1000 * 60,     // 데이터가 '신선'하다고 보는 시간(옵션)
                            // : 1분 동안은 새로고침/포커스 이동 시에도 refetch를 하지 않음
                            //   근데 1분이 지난다? 낡은 데이터라 판단해 새 데이터를 가져올려고 refetch 시도
});
